...
partial class ATT_Q_008 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!(Page.IsPostBack))
        {
            ...
            //modDB.SetFields("OVERDATE", "加班日期", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            //modDB.SetFields("OVERTMST", "加班時起", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            //modDB.SetFields("OVERTMEN", "加班時迄", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            
            //modDB.SetFields("OVERHOUR", "加班時數", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            //modDB.SetFields("OVERHR01", "待休時數", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            //modDB.SetFields("OVERHR02", "加班費時", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            ...
            //modDB.SetFields("FINTM01" , "打卡/1", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            ...
            modDB.SetFields("EMPNAME" , "員工職號/姓名", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            modDB.SetFields("VACA_NAME", "假別", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            modDB.SetFields("VATMDATE", "請假日期", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            modDB.SetFields("VATMST",   "起始時間", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            modDB.SetFields("VATMEN",   "結束時間", this.grdList, HorizontalAlign.Center, "", false, 0, false);
            modDB.SetFields("VATM_HOURS", "請假時數", this.grdList, HorizontalAlign.Center, "", false, 0, false);
    ...
    protected void btnClear_Click(object sender, EventArgs e)
    {
        Session["QryField"]= null;
        Session["iCurPage"] = null;
        Response.Redirect("ATT_Q_008.aspx");
    }
    ...
    {
        modUtil.GridView2Excel("ATTQ008.xls", this.grdList);
    }
    ...
    protected void ShowGrid()
    {
        ...
        //if (SCHECKFLAG)
        //{
        //    sSQL = "EXEC ATTQ005R '" + txtStnID.Text + "','" + DTSTSTR + "','" + DTEDSTR + "','" + txtEmpID.Text + "','" + txtEmpID.Text + "'";
        ...
        sSQL = "SELECT VATM_EMPLID + ' ' + EMPL_NAME AS EMPNAME,V.VATM_VANMID,VACA_NAME,CONVERT(char(10),VATM_DATE_ST,120) AS VATMDATE"
             + "      ,SUBSTRING(VATM_TIME_ST,1,2)+':'+SUBSTRING(VATM_TIME_ST,3,2) AS VATMST"
             + "      ,SUBSTRING(VATM_TIME_EN,1,2)+':'+SUBSTRING(VATM_TIME_EN,3,2) AS VATMEN"
             + "      ,VATM_HOURS"
             + "  FROM MP_HR.DBO.VACATM V,MP_HR.DBO.EMPLOYEE E,SCHEDM S,MP_HR.DBO.VACAMF F"
             + " WHERE EMPID=VATM_EMPLID AND V.VATM_EMPLID=E.EMPL_ID AND EMPL_DEPTID='" + txtStnID.Text + "'"
             + "  AND V.VATM_VANMID=F.VACA_ID"
             + "  AND CONVERT(char(8),VATM_DATE_ST,112)=S.SHSTDATE "
             + "  AND (SHSTDATE+SHSTTIME) > '" + PREDAYSTR + "2300'"
             + "  AND (SHSTDATE+SHSTTIME) <='" + DTEDSTR + "2300'"
             + "  AND (CONVERT(char(8),VATM_DATE_ST,112)+VATM_TIME_ST)  > '" + PREDAYSTR + "2300'"
             + "  AND (CONVERT(char(8),VATM_DATE_ST,112)+VATM_TIME_ST) <= '" + DTEDSTR + "2300'"
             + "  AND NOT((CONVERT(char(8), VATM_DATE_ST,112)+VATM_TIME_ST BETWEEN (SHSTDATE+SHSTTIME) AND (SHEDDATE+SHEDTIME)) "
             + "      AND (CONVERT(char(8), VATM_DATE_EN,112)+VATM_TIME_EN BETWEEN (SHSTDATE+SHSTTIME) AND (SHEDDATE+SHEDTIME))) "
             + " ORDER BY VATM_EMPLID,VATM_DATE_ST";
